...
# Build Redis save path for PHP sessions
file_env "REDIS_HOST"
file_env "REDIS_PASSWORD"
if [ "$REDIS_HOST" ]; then
  CONNECT_STRING="tcp://$REDIS_HOST"
  [ "$REDIS_PASSWORD" ] && CONNECT_STRING+="?auth=$REDIS_PASSWORD"
  echo -e "session.save_handler = redis\nsession.save_path = \"$CONNECT_STRING\"" > /usr/local/etc/php/conf.d/redis-session.ini
fi
